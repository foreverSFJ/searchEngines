...
public class BxwxNovelList extends AbstractGetNovelList{
	...
	public List<Novel> getsNovelList(String url, Integer maxTryTimes) {
		...
		try {
			//笔下文学的小说列表结构是tr-td,每一本小说都是一个tr，小说的具体细节在td中，所以获取所有的tr就可以了
			...
			for (int index = 1, size = trs.size(); index < size; index++) {
				...
				novel.setAddTime(DetailProcess.getPerDate());
				novels.add(novel);
			}
